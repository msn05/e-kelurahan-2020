      ...
      if(role === 2){
        ...
        TabelFilter.ajax.reload();
         ...
         let TabelFilter = $('.tabelrole').DataTable({
            ...
            "scrollY":true,
            "ajax": {
               "url": (Keterangan === 'keterangan') ? "../../../api/request_signature_lurah/show_keterangan.php" : "../../../api/request_signature_lurah/show_pajak.php",
               "type": "POST",
               "data": function ( d ) {
                 return $.extend( {}, d, {
                   "status": $('#Status').val(),
         ...
         { "data": "code_printRSL", "title": "Jenis Surat", "name": "code_printRSL" },
         { "data": "CreatedSurat", "title": "Tanggal Dibuat", "name": "CreatedSurat" },
         { "data": "SuratStatus", "title": "Status Surat", "name": "SuratStatus" },
         { "data": "office_name", "title": "Nama Staf", "name": "office_name" },
         { "data": "position", "title": "Posisi", "name": "position" },
         {"data": "id_request", "title":"Aksi", "name":"id_request", "render": function(data,type,rows){
            if(rows.SuratSataus != null){
                return ``
            }else{
              return '<button id="setuju" value="'+rows["id_request"]+'" data-load="" class="btn btn-sm btn-success text-white" title="Setujui"><i class="bx bx-check-double f"></i></button> <button id="tolak" value="'+rows["id_request"]+'" data-load="" class="btn btn-sm btn-warning text-white" title="Tolak"><i class=" bx bx-window-close "></i></button> <button id="ulangi" value="'+rows["id_request"]+'" data-load="" class="btn btn-sm btn-danger text-white" title="Ulangi"><i class="  bx bx-pencil  "></i></button>';
          }
      }
  }
  ]
           ...
           Swal.fire({
               ...
               if (result.isConfirmed) {
                  ...
                  $.ajax({
                     ...
                     url  : (Keterangan === 'keterangan') ? "../../../api/request_signature_lurah/statement_letter/update.php" : "../../../api/request_signature_lurah/tax/update.php",
                     processData:false,
                     dataType: "json",
           ...
           Swal.fire({
               ...
               if (result.isConfirmed) {
                  ...
                  $.ajax({
                     type : 'POST',
                     data : JSON.stringify(data),
                     url  : '../../../api/request_signature_lurah/statement_letter/update.php',
                     processData:false,
                     dataType: "json",
                     success: function (respone)
                     {
